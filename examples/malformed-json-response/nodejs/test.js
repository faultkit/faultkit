...
test("decideTool returns expected shape", async () => {
  ...
  try {
    ...
    const decision = await decideTool(client, "find user 42");
    assert.equal(decision.tool, "lookup_user");
    assert.equal(decision.args.id, 42);
  } finally {
    await shutdown();
  }
});
